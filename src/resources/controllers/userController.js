...
const publishPost = async (req, res) => {
    ...
    try {
        ...
        const data = await postModel.create({ owner, title, category, content });
...
const getBookmarkedPost = async (req, res) => {
    try {
        ...
        const data = await bookmarkPostModel.find({ userID }).populate('postID').lean();
        return res.status(200).json(data);
    } catch (error) {
        return res.status(500).json(error.message);
    }
...
const getFollowingUser = async (req, res) => {
    try {
        const userID = req.query.userID;
        const data = await followUserModel.find({ userID }).populate('postID').lean();
        return res.status(200).json(data);
    } catch (error) {
        return res.status(500).json(error.message);
    }
